...
# Asegura rutas relativas correctas al tejer/correr shiny
...
for (p in pkgs) {
  if (!requireNamespace(p, quietly = TRUE)) {
    stop("Falta paquete: ", p, " (instálalo y vuelve a correr).")
  }
}
...
export_dotplots_one_contrast <- function(cn, show_n = 15, out_dir = "figures", width_in = 8, height_in = 6, dpi = 300) {
...
contrast_obj <- reactive({
  results_by_contrast[[input$contrast]]
})

output$deg_dt <- renderDT({
  obj <- contrast_obj()
  DT::datatable(
...
output$go_bp_dt <- renderDT({ enrich_to_dt(contrast_obj()$go_bp, paste0("GO BP: ", input$contrast)) })
output$go_mf_dt <- renderDT({ enrich_to_dt(contrast_obj()$go_mf, paste0("GO MF: ", input$contrast)) })
output$go_cc_dt <- renderDT({ enrich_to_dt(contrast_obj()$go_cc, paste0("GO CC: ", input$contrast)) })

output$go_bp_plot <- renderPlot({ enrich_dotplot(contrast_obj()$go_bp, paste0("Dotplot GO BP: ", input$contrast), input$show_n) })
output$go_mf_plot <- renderPlot({ enrich_dotplot(contrast_obj()$go_mf, paste0("Dotplot GO MF: ", input$contrast), input$show_n) })
output$go_cc_plot <- renderPlot({ enrich_dotplot(contrast_obj()$go_cc, paste0("Dotplot GO CC: ", input$contrast), input$show_n) })

output$kegg_dt <- renderDT({
  obj <- contrast_obj()
  enr_kegg <- obj[[obj$kegg_mode]]
...
observeEvent(input$export_plots, {
  if (isTRUE(input$export_plots)) {
    export_dotplots_one_contrast(input$contrast, show_n = input$show_n)
  }
})
```